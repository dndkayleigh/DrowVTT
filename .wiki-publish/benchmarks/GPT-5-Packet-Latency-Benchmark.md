...
When names like `moves5` or `attacks6` appear below, those limits are not chosen by the model itself. The app first computes legal options from the board state, then trims what it sends in the prompt:

- `moves5` means the prompt includes only the 5 strongest legal move destinations the app found for the active creature
- `attacks6` means the prompt includes up to 6 legal attack windows the app found from those candidate positions
...
| Data artifacts | `backend/benchmark-results/packet-latency-2026-03-28-runs3.summary.json`<br>`backend/benchmark-results/packet-latency-2026-03-28-runs3.overall.csv`<br>`backend/benchmark-results/packet-latency-2026-03-28-runs3.runs.csv`<br>`backend/benchmark-results/packet-latency-2026-03-28-runs3.raw.log` |
...
| `compact_moves5` | `compact_base` plus only the top 5 legal move destinations precomputed by the app before the model sees the prompt. |
| `compact_attacks6` | `compact_base` plus up to 6 legal attack windows precomputed by the app before the model sees the prompt. |
| `compact_summary` | `compact_base` plus summarized active-token statblock instead of the full statblock. |
| `compact_moves5_attacks6` | `compact_base` plus the top 5 legal move destinations and up to 6 legal attack windows, both precomputed by the app. |
| `compact_moves5_summary` | `compact_base` plus the top 5 legal move destinations and a summarized statblock. |
| `compact_moves5_attacks6_summary` | `compact_base` plus the top 5 legal move destinations, up to 6 legal attack windows, and a summarized statblock. |
...
![duel-goblin-vs-acolyte](../assets/scenario-screenshots/duel-goblin-vs-acolyte.png)
...
![ranged-bandit-crossfire](../assets/scenario-screenshots/ranged-bandit-crossfire.png)
...
**What happened in the archived turn:** "The bandit advanced from (2,2) to (5,5) to pressure the low-AC caster and then fired a legal light crossbow shot at the Acolyte."
...
![crowded-ogre-frontline](../assets/scenario-screenshots/crowded-ogre-frontline.png)
...
![air-elemental-flank](../assets/scenario-screenshots/air-elemental-flank.png)
...
**What happened in the archived turn:** "The air elemental repositioned diagonally and then followed through with a legal slam attack."
...
![boss-dragon-vs-party](../assets/scenario-screenshots/boss-dragon-vs-party.png)
...
![aboleth-control-web](../assets/scenario-screenshots/aboleth-control-web.png)
...
- `backend/measure-packet-accuracy.mjs`
- `backend/ai-turn-eval-utils.mjs`
