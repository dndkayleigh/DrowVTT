export function buildTokenSelectionNote({
  ...
  if (strategyId === 'group_tactical') {
    return groupCount
      ? `${strategyLabel} will use ${groupCount} grouped AI-controlled token${groupCount === 1 ? '' : 's'}. Use ctrl/cmd-click on desktop or Group Select on mobile, then click Set Group From Selection.`
      : `${strategyLabel} needs an explicit AI-controlled group. Use ctrl/cmd-click on desktop or Group Select on mobile, then click Set Group From Selection.`;
  }
