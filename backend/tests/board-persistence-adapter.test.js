...
test('local board persistence adapter saves and replaces named slots', async () => {
  ...
  const first = await adapter.saveSlot({
    name: '  Round 3 Start  ',
  ...
  const second = await adapter.saveSlot({
    id: 'slot-1',
    name: 'Round 4 Start',
  ...
  const snapshot = await adapter.loadSaveSlotSnapshot('slot-1');
  assert.equal(snapshot.state.round, 4);
});

test('local board persistence adapter appends and clears autosaves', async () => {
  let idCounter = 0;
  const adapter = createLocalBoardPersistenceAdapter({
    storage: createMemoryStorage(),
    snapshotStore: createMemorySnapshotStore(),
  ...
  const first = await adapter.appendAutosave({
    version: 1,
    savedAt: '2026-04-02T12:00:00.000Z',
    state: { round: 1 }
  });
  const second = await adapter.appendAutosave({
    version: 1,
  ...
  const snapshot = await adapter.loadAutosaveSnapshot('entry-2');
  assert.equal(snapshot.state.round, 2);

  await adapter.clearAutosaveHistory();
...
test('local board persistence adapter deletes slots and supports custom storage keys', async () => {
  ...
  const removed = await adapter.deleteSaveSlot('slot-1');
